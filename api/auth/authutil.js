...
authUtilMethods = {
  ...
  generateSecurityQues: (req, res, next) => {
    ...
    let securityQues = fs.readFileSync(
      process.env.HOME + "\\desktop" + "\\securityques.json"
    ); // Replace this path with whichever path you save your security.json file
  ...
  encryptData: (req, res, next) => {
    ...
    switch (originalUrlContent[4]) {
      ...
      case "firstLogin":
        ...
        Users.findOneAndUpdate({ userId: req.body.userId }, { initPwd: "" })
          .exec() // Destroy the initial password (initPwd)
          .then(foundUser => {
    ...
    if (typeof token !== "undefined") {
      jwt.verify(token, "secretKey", (jwtErr, decode) => {
        if (typeof decode !== "undefined" && !jwtErr) {
          ...
          //     default:
          //         res.status(403).json({'result': 'Something just broke'});
          //     break;
          // }
